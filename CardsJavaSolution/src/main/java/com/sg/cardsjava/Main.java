...
public class Main {
    ...
    public static boolean isFlush(HashSet<Card> hand) {
        Set<Suit> suits = hand.stream().map((c) -> c.getSuit())
                .collect(Collectors.toSet());
        return suits.size() == 1;
    }

    public static boolean isStraight(HashSet<Card> hand) {
        List<Integer> ranks = hand.stream().map(c -> c.getRank().getVal())
                .sorted()
    ...
    public static boolean isFourOfAKind(HashSet<Card> hand) {
        Set<Rank> ranks = hand.stream().map((c) -> c.getRank())
                .collect(Collectors.toSet());
        return ranks.size() == 1;
    }

    public static boolean isThreeOfAKind(HashSet<Card> hand) {
        Map<Rank, Long> rankMap = hand.stream().map((c) -> c.getRank())
                .collect(Collectors.groupingBy(c -> c, Collectors.counting()));
        
        return rankMap.values().stream().anyMatch((c) -> c == 3);
    }
    ...
    public static boolean isTwoPair(HashSet<Card> hand) {
        ...
        return rankMap.values().stream().allMatch((c) -> c == 2);
    }
